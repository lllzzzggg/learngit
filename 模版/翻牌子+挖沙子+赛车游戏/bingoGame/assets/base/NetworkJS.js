var NetworkJS = cc.Class({
	...
	sendXHR: function (racingjs) {
		...
		//题库
		// var fileUrl =  cc.url.raw('resources/bingoGame.json');
	...
	gameLoadSuccess: function (totalNumber) {
		// if (!CONSOLE_LOG_OPEN) {
			cc.log("gameLoadSuccess");
		var params = encodeURI('totalNumber=' + totalNumber);
		window.location.href = 'optionBlank://gameLoadSuccess?' + params;
		// }
	},

	setTotalTime: function(totalNum){
		this.totalTime = totalNum;
	},

	//通知游戏结束
	gameOver: function (answerInfoArr) {
		var data = encodeURI(JSON.stringify(answerInfoArr));
		if (CONSOLE_LOG_OPEN) console.log('totalTime='+this.totalTime+'&dataJson=' + JSON.stringify(answerInfoArr));
		window.location.href = 'optionBlank://gameOver?status=1&totalTime='+this.totalTime+'&data=' + data;
		// }
	},
	//通知游戏加载成功，开始游戏
