...
cc.Class({
    ...
    initPage(data,right,audio,url,time,pageName){
        let self = this;
        if(data){
    ...
    bindSelect(){
        ...
        let choice_words = select.parent.parent.getComponent('choice_words')
            ...
            select.on('touchend',()=>{
                ...
                if(self.data === self.right){
                    ...
                    for(let i = 0;i<children.length;i++){
                        let page_js = children[i].getComponent('select_words');
