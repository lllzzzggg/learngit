cc.Class({
    ...
    initWords() {
        ...
        if (words && words.length !== 0) {
            ...
            for (let i = 0; i < words.length; i++) {
                ...
                word_pane.getComponent('cc.RichText').string = `<color=#5d369f>${ word }</color>`;
                pane.addChild(word_pane)
                word_pane.x = 0;
                word_pane.y = -72 - 144 * i
                let word_node = cc.instantiate(self.word);
                let text = cc.find('button/text', word_node);
                text.getComponent('cc.RichText').string = `<color=#5d369f>${ word }</color>`;
                text.active = false
    ...
    bindDragon() {
        ...
        let dragon_node = cc.find('word_bar/word_card_Icon_ske', self.node);
        let dragonBones = dragon_node.getComponent('dragonBones.ArmatureDisplay');
        let hand_node = cc.find('word_bar/Guide_click_ske', self.node)
        let words_node = cc.find('words', self.node);
        ...
        let clicked_words = cc.sys.localStorage.getItem(`${self.server}_${self.game}_clicked_words`) || ''
        if(clicked_words !== ''){
            hand_node.active = false
            dragonBones.playAnimation('static', -1);
        }else{
        ...
        if (dragon_node) {
            ...
            //dragonBones.playAnimation('motion', -1);
            button.on('touchend', () => {
                if(self.configData.words[self.currentIndex].length !== 0){
                ...
                self.scheduleOnce(() => {
                    for (let i = 0; i < children.length; i++) {
                        let word = children[i];
                        let clicked_words = cc.sys.localStorage.getItem(`${self.server}_${self.game}_clicked_words`) || ''
                        let repeatAction = cc.repeatForever(cc.sequence(cc.scaleTo(1, 1.2), cc.scaleTo(1, 1)));
                        let word_text = word.getComponent('cc.RichText').string
                        if(!(clicked_words && clicked_words.indexOf(word_text) !== -1)) {
                        ...
                        word.on(cc.Node.EventType.TOUCH_END, () => {
                            words_node.children.map(child => {
                                ...
                                if(word_text === text){
                                    cc.sys.localStorage.setItem(`${self.server}_${self.game}_clicked_words`, `${ clicked_words }|${text}`)
                                    word.stopAllActions()
