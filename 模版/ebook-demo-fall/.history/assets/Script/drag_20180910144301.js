cc.Class({
    ...
    initPictures() {
        ...
        let page_text = cc.find('scrolls/page', self.node)
        ...
        if (pictures) {
            ...
            if(pictures.length > 6){
                ...
                left.active = false;
                right.active = true;
                page_text.getComponent('cc.RichText').string = '1/2'
            }else{
                self.addPictures(content, pictures,'part1', 0)
                left.active = false;
                right.active = false;
                page_text.getComponent('cc.RichText').string = '1/1'
            }
            self.triggerPage();
    ...
    addPictures(content, pictures, part, prevWidth, hidden){
        ...
        for (let i = 0; i < pictures.length; i++) {
            ...
            if (item.img) {
                //需实现服务器加载
                self.loadImg1(item.img, picture);
            }
            //数据保留
    ...
    triggerPage(){
        ...
        right.on(cc.Node.EventType.TOUCH_END, (event) => {
            ...
            page_text.getComponent('cc.RichText').string = '2/2'
        })
    },
    hiddenPictures(part){
        let self = this;
    ...
    calcPoint(crease){
        let self = this
        crease ? self.dragTime++ : self.dragTime
    ...
    loadImg1(imgUrl, node) {
        let server = cc.sys.localStorage.getItem('server');
        let host = cc.sys.localStorage.getItem('HOST');
        let page = this.configData.game;
        cc.loader.load(`${host}${server}/${page}/img/${imgUrl}.jpg`, (err, img) => {
            if (img) {
                cc.find('img', node).getComponent(cc.Sprite).spriteFrame = new cc.SpriteFrame(img);
            }
        })
    },
