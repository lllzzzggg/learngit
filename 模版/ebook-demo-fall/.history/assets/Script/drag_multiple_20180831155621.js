cc.Class({
    ...
    initDrags(data) {
        ...
        blocks.removeAllChildren()
         //如果之前保留过做过的题，获取初始化
         self.setRightNodes(data,len)
         

        if(self.review === 'true'){
            self.setWrongNodes(data,len)
        }
        self.activeDrags()
        self.activePageButton()
    },
    setRightNodes(data,len){
        let self = this
        let blocks = cc.find(`blocks/${self.type}s`, self.node)
    ...
    setWrongNodes(data,len){
        let self = this
        let correct_nodes = cc.sys.localStorage.getItem(`${self.server}_${self.data.game}_wrong_nodes`) || ''
        let local_nodes = correct_nodes !== '' ? correct_nodes.split('|') : []
        for (let i = 0; i < len; i++) {
            let drag = data[i]
            let content = drag.content
