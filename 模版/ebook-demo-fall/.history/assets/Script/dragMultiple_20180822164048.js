cc.Class({
    ...
    getPositionFromWorld(node, box){
        ...
        let nodeX = node_world_point.x - box_world_point.x + node.width * ((node.parentBox.indexOf('img') !== -1) ? 0.6 : 1) / 2 - groove.width / 2
        let nodeY = node_world_point.y - box_world_point.y + node.height * ((node.parentBox.indexOf('img') !== -1) ? 0.6 : 1) / 2 - groove.height / 2
    },
    validateDrags(){
        let self = this
