cc.Class({
    ...
    getPositionFromWorld(node, box){
        ...
        let nodeX = node_world_point.x - box_world_point.x + node.width * ((node.parentBox.indexOf('img') !== -1) ? 0.6 : 1) / 2 - box.width / 2
        let nodeY = node_world_point.y - box_world_point.y + node.height * ((node.parentBox.indexOf('img') !== -1) ? 0.6 : 1) / 2 - box.height / 2
    },
    validateDrags(){
        let self = this
