...
cc.Class({
    ...
    initPage(data){
        ...
        time_node.getComponent('cc.RichText').string = self.formatText() + self.formatTime(endTime - startTime);
        answer_node.getComponent('cc.RichText').string = '完成答题: ' + total + '次';
        open_node.getComponent('cc.RichText').string = '开口次数: ' + openTime + '次';
        rate_node.getComponent('cc.RichText').string = '答题成功: ' + (Math.round((finishTime / total).toFixed(2) * 100) || 0) + '%'
