cc.Class({
    ...
    initDrags(data) {
        ...
        blocks.removeAllChildren()
         //如果之前保留过做过的题，获取初始化
         let correct_nodes = cc.sys.localStorage.getItem(`${self.host}${self.server}_${self.data.game}_correct_nodes`) || ''
    ...
    initGrooves(data){
        ...
        for(let i = 0; i < len; i++){
            ...
            if(self.type === 'img'){
                ...
                text_node.string = tip !== '' ? `<color=#B76558>${ tip }</color>` : ''
    ...
    loadImg(imgUrl, node) {
        let self = this
        cc.loader.load(`${self.server}/${self.game}/img/${imgUrl}.jpg`, (err, img) => {
            if (err) return
            if (img) {
