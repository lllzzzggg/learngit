cc.Class({
    ...
    getPositionFromWorld(node, box){
        ...
        let nodeX = node_world_point.x - groove_world_point.x + node.width * ((node.parentBox.indexOf('img') !== -1) ? 0.6 : 1) / 2 - groove.width / 2
        let nodeY = node_world_point.y - groove_world_point.y + node.height * ((node.parentBox.indexOf('img') !== -1) ? 0.6 : 1) / 2 - groove.height / 2
    },
    validateDrags(){
        let self = this
