    ...
    initWords() {
        ...
        if (words && words.length !== 0) {
            ...
            for (let i = 0; i < words.length; i++) {
                ...
                word_pane.getComponent('cc.RichText').string = `<outline color=#000557 width=8><color=#5d369f>${ word }</color></outline>`;
                pane.addChild(word_pane)
                word_pane.x = 0;
                word_pane.y = -50 - 100 * i
                let word_node = cc.instantiate(self.word);
                let text = cc.find('button/text', word_node);
                text.getComponent('cc.RichText').string = `<outline color=#000557 width=8><color=#5d369f>${ word }</color></outline>`;
                text.active = false
