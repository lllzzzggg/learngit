...
cc.Class({
        ...
        if(data){
            ...
            if(!lastAnswer){
                for(let i = 0;i<selects.length;i++){
                    let page_js = selects[i].getComponent('select_sentence_many');
                    page_js.bindSelect();
                }
            }else{
                self.lastData()
    ...
    loadDynamicResources(data){ //提前加载页面时，需要加载出图片资源，
        ...
        if(data){
            self.configData = data;
            self.img = self.server + self.configData.game + self.configData.look;
            self.readAudio = self.server + self.configData.game + self.configData.audio;
            self.selection = self.configData.selection;
            self.rightArr = self.configData.right   
            self.rightTime = self.configData.rightTime
    ...
    lastData(){
        ...
        for(let i = 0;i<children.length;i++){
            ...
            let text = children[i].getChildByName('selection').getComponent(cc.Label).string
    ...
    initSelection(options){
        ...
        if(options.length !== 0){
            for(let i = 0;i<options.length;i++){
                ...
                let select_js = select.getComponent('select_sentence_many');
                select_js.initPage(option);
    ...
    pushSelect(add,select){
        let self = this;
        //let word = add.getChildByName("text").getComponent(cc.Label).string
        
        self.selectArray.push(select);
        self.selectNode.push(add);
        self.submitSelect()
    },
    deleteSelect(pop,select){
        let self = this
        // let word = pop.getChildByName("text").getComponent(cc.Label).string
        let a = self.selectArray.indexOf(select);
    ...
    rightSelect(){
        ...
        let children = cc.find('selection',self.node).children;
        for(let i = 0;i < children.length;++i){
            let selection = children[i].getChildByName("selection").getComponent(cc.Label).string;
            let right = children[i].getChildByName("judge").getChildByName("right")
            let a = self.rightArr.indexOf(selection)
            if(a >= 0){
                right.opacity = 255;
            }
        }
