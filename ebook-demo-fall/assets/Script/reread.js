...
cc.Class({
    ...
    mcphone(){
        ...
        recording_size.on('touchend',()=>{
            ...
            dragonRecording.playAnimation('None',1)
            self.unscheduleAllCallbacks()
            self.endVoiceTest()
            self.clickPlayAudio()
        })
    ...
    minTimeout(){
        ...
        let submit = cc.find('submit',self.node)
        let recording = cc.find('recording',self.node)
        let timeTitle = cc.find('timeout',self.node)
        let dragonRecording = recording.getComponent("dragonBones.ArmatureDisplay");
        
        self.timeTie = function(){ 
            if(self.ifRev == 'true'){
                submit.active = true;
            }
            recording.active = false;
            timeTitle.active = true;
            timeTitle.runAction(cc.fadeIn(1))
            dragonRecording.playAnimation('None',1) 
        }
        self.fadeout = function(){ 
            timeTitle.runAction(cc.fadeOut(1))
        }
        self.activeF = function(){ 
            timeTitle.active = false;
            self.endVoiceTest()
            self.clickPlayAudio()
        }
        self.schedule(self.timeTie,60,0,0)
        self.schedule(self.fadeout,63,0,0)
        self.schedule(self.activeF,64,0,0)   
    },
    startVoiceTest(){
        let self = this;
        self.flag = false;
    ...
    handleResult(result){
        ...
        let timer = setInterval(()=>{
            ...
            if(result){
                ...
                // result = eval('(' + result + ')')
                    console.log(result)
                    window.Vipx.word.getComponent('reread').tokenId = result.tokenId;
                    window.Vipx.word.getComponent('reread').audioUrl = result.audioUrl;
                    window.Vipx.word.getComponent('reread').showResult(result.result,result.audioUrl,result.tokenId);
                    window.Vipx.word.getComponent('reread').saveResult(result)   //保存结果
            }else{
    ...
    bindCheck(result,audioUrl,tokenId){
        ...
        let score = result.score || result.overall;
        console.log(score)
