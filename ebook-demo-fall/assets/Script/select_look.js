...
cc.Class({
    ...
    initPage(data,right,audio,page,time){
        let self = this;
        if(data){
            self.data = data;
            self.right = right;
    ...
    bindSelect(){
        ...
        if(select){
            ...
            select.on('touchend',()=>{
                ...
                let choice_look = select.parent.parent.getComponent('choice_look');
                ...
                if(self.data === self.right){
                    ...
                    for(let i = 0;i<children.length;i++){
                        let page_js = children[i].getComponent('select_look');
                        page_js.unscheduleAllCallbacks();
                    }
                    local.saveResult(self.page,result,'choice')
                    if(ifReview == 'true'){
                        cc.sys.localStorage.setItem('totalScore', (+totalScore + 100));
                    }else{
                        choice_look.totalScore()
                    }
                }else{
                    choice_look.des();
                    read.off('touchend');
    ...
    reviewSubmit(){
        ...
        self.scheduleOnce(()=>{
            ...
            for(let i = 0;i<selects.length;i++){
                ...
                if(selects[i].getChildByName("word").getComponent(cc.RichText).string === self.right){   
                    selects[i].getChildByName('right').getChildByName('look_right').opacity = 0
                    selects[i].getChildByName('right').opacity = 255
                }
                read.on('touchend',()=>{
                    cc.audioEngine.stopAll();
    ...
    previewSubmit(){
        ...
        self.tryFull = () =>{ 
            for(let i = 0;i<selects.length;i++){
                let page_js = selects[i].getComponent('select_look');
                page_js.bindSelect();
            }
    ...
    audioPlay(){
        let self = this;
        let read = self.node.parent.parent.getChildByName('read')
