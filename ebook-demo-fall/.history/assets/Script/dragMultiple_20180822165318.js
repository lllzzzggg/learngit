cc.Class({
    ...
    getPositionFromWorld(node, box){
        ...
        console.log(node_world_point)
        console.log(box_world_point)
        let nodeX = node_world_point.x - box_world_point.x + node.width / 2 - box.width / 2
        let nodeY = node_world_point.y - box_world_point.y + node.height / 2 - box.height / 2
    ...
    dropDownToBlocks(node){
        ...
        if(self.type === 'img'){
            ...
            //node.y = -position.y
            parent.addChild(node)
            node.runAction(cc.spawn(cc.moveTo(), cc.scaleTo(0.2, (node.parentBox.indexOf('img') !== -1) ? 0.6 : 1));
            ...
            text.x = node.x
            text.y = node.y
            let position = text.convertToNodeSpaceAR(parent.getPosition())
            text.opacity = 0;
