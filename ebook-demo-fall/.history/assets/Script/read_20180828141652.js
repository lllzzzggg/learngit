cc.Class({
    ...
    initLabel() {
        ...
        content.getComponent('cc.RichText').string = self.highlightWords(str, words);
    },
    highlightWords(str, words){
        for(i = 0;i < words.length;i++){
            let word = words[i]
            console.log(word)
            console.log(new RegExp(`/${ word }/g`))
            str = str.replace(new RegExp(`/${ word }/g`), `<color=#FFF604>${ word }</color>`)
        }
        console.log(str)
        return str
    },
