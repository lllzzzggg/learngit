cc.Class({
    ...
    initPage(data) {
        ...
        if (data) {
            ...
            self.successTime = 0;
            self.blankGrooves = [];
            self.local = cc.sys.localStorage.getItem(`${self.server}_${self.configData.game}_pre_drag`) || 'false'
    ...
    initPictures() {
        ...
        if (pictures) {
            ...
            if(pictures.length > 6){
                ...
                self.addPictures(content, pictures, 0)
                left.active = false;
                right.active = false;
            }
            self.triggerPage();
    ...
    addPictures(content, pictures, part, prevWidth, hidden){
        ...
        for (let i = 0; i < pictures.length; i++) {
            ...
            if(self.local === 'true'){
                ...
                //如果是作业且全部拖拽正确
                let review = 
                if (cc.sys.localStorage.getItem('review') === 'true' && cc.sys.localStorage.getItem(`${self.server}_${self.configData.game}_rev_drag`) === 'true') {
            ...
            if (self.mode === 'regular') {
                ...
                if (judge) {
                    ...
                    if (self.successTime === self.dragSuccessTime) {
                        ...
                        //拖拽完成，保留数据
                        cc.sys.localStorage.setItem(`${self.server}_${self.configData.game}_pre_drag`, true)
                        //统计完成情况
                        ...
                        cc.sys.localStorage.setItem(`${self.server}_${self.configData.game}_pre_drag`, false)
                    }
