...
cc.Class({
    ...
    initPage(data){
        ...
        time_node.getComponent('cc.Label').string = '本次用时: ' + self.formatTime(endTime - startTime);
        answer_node.getComponent('cc.Label').string = '完成答题: ' + total + '次';
        open_node.getComponent('cc.Label').string = '开口次数: ' + openTime + '次';
        rate_node.getComponent('cc.Label').string = '答题成功: ' + (Math.round((finishTime / total).toFixed(2) * 100) || 0) + '%'
    ...
    formatText(title,content){
        return `<color=#FFFFFF>Rich</c><color=#FFD408>Text</color>`
    },
    formatTime(time){
        time = time/1000;
