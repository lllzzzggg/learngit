cc.Class({
    ...
    start() {
        ...
        cc.loader.loadRes('config.json', (err, res) => {
            if (err) return;
            self.initGame(res);
        });
    },
    initGame(pages) {
        ...
        if (pages) {
            ...
            // }
            cc.sys.localStorage.setItem('HOST', 'http://10.32.31.81/resources/')
    ...
    scoreNumFun(){
        ...
        let scorePrefab = ['reread','choice_sentence_many','choice_words_many','choice_look_many','choice_listen_many','choice_listen','choice_look','choice_words','choice_sentence','read','drag']
        let scoreNums = 0;
                ...
                for (let i = 0; i < len; i++) {
                    cc.loader.load(audioArr[i], function (err, result) {
                        ...
                        if (self.num == 0) {
                            ...
                            cc.audioEngine.stopAll();
                            loading.opacity = 255;
                            loading.getComponent('cc.Label').string = '正在加载资源...' + Math.round((len - self.num) / len * 100) + '%';
                        }
                        //}
                    });
                }
