cc.Class({
        ...
        node.on(cc.Node.EventType.TOUCH_END, event => {
            ...
            if(right_node_name !== ''){
                ...
                if(self.type === 'img'){
                    ...
                    let position = self.getPositionFromWorld(node,)
                    node.x = -position.x
                    node.y = -position.y
    ...
    getPositionFromWorld(node, box){
        ...
        let nodeX = node_world_point.x - box_world_point.x + node.width * ((node.name === 'img') ? 0.6 : 1) / 2 - box.width / 2
        let nodeY = node_world_point.y - box_world_point.y + node.height * ((node.name === 'img') ? 0.6 : 1) / 2 - box.height / 2
