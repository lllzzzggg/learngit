cc.Class({
    ...
    initTitle(title){
        ...
        let titleAudio = `${self.host}${self.server}/${self.game}/audio/${self.audio}.mp3`
        let correct_nodes = cc.sys.localStorage.getItem(`${self.server}_${self.data.game}_correct_nodes`) || ''
        let wrong_nodes = cc.sys.localStorage.getItem(`${self.server}_${self.data.game}_wrong_nodes`) || ''
        self.scheduleOnce(() => {
            if(!(self.review === 'true' && (correct_nodes !== '' || wrong_nodes !== ''))){
                cc.audioEngine.stopAll()
                 cc.audioEngine.play(titleAudio, false, 1)
            }
        },0.2)
    ...
    initDrags(data) {
        ...
        let tips = cc.find('tips', self.node)
        let showTips = cc.sys.localStorage.getItem('show_tips') || ''
        if(showTips === 'false'){
            tips.active = false
        }
        blocks.active = true
        ...
        blocks.removeAllChildren()
         //如果之前保留过做过的题，获取初始化
         let correct_nodes = cc.sys.localStorage.getItem(`${self.server}_${self.data.game}_correct_nodes`) || ''
         let local_nodes = correct_nodes !== '' ? correct_nodes.split('|').slice(1) : []

         let local_wrong_nodes = []
         let wrong_nodes = ''
         if(self.review === 'true'){
            wrong_nodes = cc.sys.localStorage.getItem(`${self.server}_${self.data.game}_wrong_nodes`) || ''
            local_wrong_nodes = wrong_nodes !== '' ? wrong_nodes.split('|').slice(1) : []
         }
        for(let i = 0;i < len; i++){
            let drag = data[i]
            ...
            if(local_nodes.length !== 0){
                //将drags中拖拽过的元素去掉
                local_nodes.map(local => {
                    let [index, position] = local.split('_')
                    if (i === +index) {
                        content = ''
                    }
                })
            }
            if (local_wrong_nodes.length !== 0) {
                local_wrong_nodes.map(local => {
                    let [index, position] = local.split('_')
                    if (i === +index) {
                        content = ''
                    }
            ...
            if(self.type === 'img'){
                ...
                if (content !== '') {
                    let img_node = self.createImgNode(content, drag.targets)
                    img_node.originIndex = i
                    img_node.scaleX = 0.6
                    img_node.scaleY = 0.6
                    if (!(self.review === 'true' && (correct_nodes !== '' || wrong_nodes !== ''))) {
                ...
                text_box.originIndex = i
                text_box.y = 0
                text_box.x = i % 2 == 0 ? -320 : 320
                if (content !== '') {
                    let text_node = self.createTextDragNode(content, drag.targets)
                    text_node.originIndex = i
                    if (!(self.review === 'true' && (correct_nodes !== '' || wrong_nodes !== ''))) {
    ...
    activePageButton() {
        ...
        tips.on(cc.Node.EventType.TOUCH_END, () => {
            tips.runAction(cc.fadeOut(0.2))
            cc.sys.localStorage.setItem('show_tips', 'false')
        })
    ...
    initGrooves(data){
        ...
        //如果之前保留过做过的题，获取初始化
        let correct_nodes = cc.sys.localStorage.getItem(`${self.server}_${self.data.game}_correct_nodes`) || ''
        let local_nodes = correct_nodes !== '' ? correct_nodes.split('|') : []

        let local_wrong_nodes = []
        let wrong_nodes = ''
        if (self.review === 'true') {
            wrong_nodes = cc.sys.localStorage.getItem(`${self.server}_${self.data.game}_wrong_nodes`) || ''
            local_wrong_nodes = wrong_nodes !== '' ? wrong_nodes.split('|') : []
        }
        ...
        for(let i = 0; i < len; i++){
            ...
            let tip = groove.tip
            let hasDrag = false
            let wrong_node = false
            if(local_nodes.length !== 0){
                local_nodes.map(local => {
                    let [index, position] = local.split('_')
                    if(i === +position){
                        hasDrag = true
                        content = self.data.drags[index].content
                    }
                })
            }
            ...
            if(self.type === 'img'){
                ...
                text_node.string = (tip&&tip !== '') ? `<color=#B76558>${ tip }</color>` : ''
                
            }else{
                groove_node = cc.find(`center/${ center }/grooves/${ len > 5 ? ('text_groove' + i) : ('text_groove_high' + i) }`, self.node)
            }
            groove_node.children.map(child =>{ if(child.name !== 'groove_text') child.removeFromParent()})
            ...
            if(content !== ''){
                let new_node = self.type === 'img' ? self.createImgNode(content) : self.createTextNode(content, len)
                if(self.review === 'true'){
                    (hasDrag && wrong_node) ? self.activeBorder(new_node, 'wrong') : self.activeBorder(new_node, 'right')
                }else{
                    hasDrag && self.activeBorder(new_node, 'right')
                }
                
                
                groove_node.addChild(new_node)
            }else{
        ...
        node.on(cc.Node.EventType.TOUCH_END, event => {
            ...
            if(right_node_name !== ''){
                ...
                if(self.type === 'img'){
                    ...
                    text.originIndex = node.originIndex
                    text.x = node.x
                    text.y = node.y
    ...
    validateDrags(){
        ...
        grooves.map(groove => {
            ...
            if(node.parentBox){
                ...
                let index = -1
                node.targets.map((target,ind) => {
                    if(target === num) index = ind
                })
                if(index == -1){
                    flag = true
                    wrongNum++
                    ...
                    let wrong_nodes = cc.sys.localStorage.getItem(`${self.server}_${self.data.game}_wrong_nodes`) || ''
                    wrong_nodes += `|${node.originIndex}_${num}`
                    cc.sys.localStorage.setItem(`${self.server}_${self.data.game}_wrong_nodes`, wrong_nodes)
                    ...
                    let correct_nodes = cc.sys.localStorage.getItem(`${self.server}_${self.data.game}_correct_nodes`) || ''
                    correct_nodes += `|${node.originIndex}_${num}`
                    cc.sys.localStorage.setItem(`${self.server}_${self.data.game}_correct_nodes`, correct_nodes)
        ...
        if(flag){
            ...
            if(self.review === 'false'){
                if(wrongNum === 1){
                    ...
                    cc.sys.localStorage.setItem('totalScore', +point + 80)
                }else if(wrongNum === 2){
                    let point = cc.sys.localStorage.getItem('totalScore') || 0
                    cc.sys.localStorage.setItem('totalScore', +point + 70)
                }else if(wrongNum > 2){
                    let point = cc.sys.localStorage.getItem('totalScore') || 0
                    cc.sys.localStorage.setItem('totalScore', +point + 60)
                }
            }
        }else{
    ...
    createTextNode(data, len){
        ...
        if(self.hasTips){
            ...
            right_btn.y = -right_btn.y + 25
            right_btn.x -= 25
            wrong_btn.y = -wrong_btn.y + 25
            wrong_btn.x -= 25
        }
        return text_node
    },
    ...
    loadImg(imgUrl, node) {
        let self = this
        cc.loader.load(`${self.host}${self.server}/${self.game}/img/${imgUrl}.jpg`, (err, img) => {
            if (err) return
            if (img) {
                node.getComponent(cc.Sprite).spriteFrame = new cc.SpriteFrame(img)
            }
        })
        // cc.loader.loadRes(`falls/drags/${imgUrl}.jpg`, (err, img) => {
        //     if(err) return 
        //     if(img){
        //         node.getComponent(cc.Sprite).spriteFrame = new cc.SpriteFrame(img)
        //     }
